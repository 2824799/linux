...
static void decode_work(struct work_struct *work)
{
	...
	if (!c->header) {
		...
		/* The picture geometry is known from here on. H.264 carries its
		 * frame rate in the VUI, which this frontend does not parse, so
		 * assume the panel rate: a decoder that cannot keep up with the
		 * display would drop frames anyway.
		 */
		mtk_vcp_vdec_hw_set_perf(c->dev->hw, c->pic.width, c->pic.height,
					 60);
		goto finish;
	}
	ret = collect_events(c);
